...
#[test]
fn o3_vectorizes_where_with_unary_on_second_array_load() {
    if let Err(reason) = armfortas::testing::native_vectorizer_support() {
        eprintln!(
            "\nHARNESS_SKIP suite=vectorize_where_unary_on_b test=o3_vectorizes_where_with_unary_on_second_array_load count=1 reason=\"{}\"",
            reason
        );
        return;
    }
    let source = fixture("do_loop_vectorize_where_unary_on_b.f90");
    ...
    assert!(
        o3_ir.contains("vneg"),
        "expected vneg lifted from `c = -d` then arm:\n{}",
        o3_ir
    );
