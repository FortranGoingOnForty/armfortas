...
/// x86 allocator policy (bug B): linear-scan at EVERY opt level, naive
/// only when forced. The naive spill-everything allocator gives every
/// vreg its own stack slot, bloating frames ~6x and overflowing the stack
/// on deep recursion (fortsh's executor SIGSEGV'd at depth ~62) —
/// gfortran does real register allocation at -O0 too. `opt_level` is
/// taken to make the "no opt-level gate" invariant explicit and testable:
/// re-adding an `opt_level >= O1` condition here would resurrect bug B,
/// and `x86_linear_scan_at_every_opt_level` would catch it.
...
mod tests {
    ...
    /// Bug B regression guard: the x86 backend must use linear-scan at
    /// EVERY opt level (the naive spill-everything allocator overflows the
    /// stack on deep recursion). Re-introducing an opt-level gate here is
    /// the exact way to regress bug B; this test forbids it.
    #[test]
    fn x86_linear_scan_at_every_opt_level() {
        for opt in [
            ...
            assert!(
                ...
                "x86 must use linear-scan at {opt:?} (bug B)"
            );
            assert!(
