...
#[test]
fn o3_vectorizes_elementwise_sqrt() {
    if let Err(reason) = armfortas::testing::native_vectorizer_support() {
        eprintln!(
            "\nHARNESS_SKIP suite=vectorize_sqrt test=o3_vectorizes_elementwise_sqrt count=1 reason=\"{}\"",
            reason
        );
        return;
    }
    let source = fixture("do_loop_vectorize_sqrt.f90");
    ...
    // Expect both an f32 vsqrt (4-lane) and an f64 vsqrt (2-lane).
    let count = o3_ir.matches("vsqrt").count();
    assert_eq!(count, 2, "expected two vsqrt:\n{}", o3_ir);
    ...
    // sqrt(1)=1, sqrt(4)=2, sqrt(16)=4 in both lines.
    for line in &trimmed {
        assert!(
            line.starts_with("1."),
