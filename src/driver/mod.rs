...
fn collect_cli_warnings(opts: &mut Options, unknown_warning_flags: &[String]) {
    ...
    if opts.openmp {
        opts.cli_warnings.push(
            "-fopenmp enables preview OpenMP support; scalar-shared PARALLEL regions are implemented and unsupported constructs or clauses are errors"
                .into(),
        );
...
LANGUAGE:
  ...
  -fopenmp                    Enable preview OpenMP support (scalar-shared PARALLEL regions)
