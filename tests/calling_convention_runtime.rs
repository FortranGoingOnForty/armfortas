...
fn specific_intrinsic_procedure_targets_keep_fortran_abi_at_every_opt_level() {
  ...
  abstract interface
    ...
    double precision function double_complex_real_callback(value)
      complex(8), intent(in) :: value
    end function double_complex_real_callback
  end interface
  intrinsic :: sin
  type :: callback_holder
...
program p
  use callback_types
  implicit none
  intrinsic :: sin, dsin, iabs, len, conjg, dconjg, dreal
  ...
  procedure(double_complex_real_callback), pointer :: double_complex_real_target
  type(callback_holder) :: holder
  complex :: result
  ...
  double_complex_real_target => dreal
  holder%rebound => sin
  ...
  if (abs(double_complex_real_target((3.5d0, -7.0d0)) - 3.5d0) > 1.0d-12) error stop 12
  print *, 'ok'
end program p
"#,
    );
