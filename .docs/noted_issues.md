...
- **(D)** GVN, SROA, GlobalLsf, BCE — implementations exist (`src/opt/gvn.rs`, `sroa.rs`, `global_lsf.rs`, `alias.rs`, `bce.rs`) with passing unit tests, but disabled from pipeline due to interaction bugs: (1) SROA + second Mem2Reg triggers unswitching SSA violations on nested loops; (2) GlobalLsf forwards stores across calls that clobber memory (fibonacci miscompile); (3) SROA rejects byte-level GEPs but array constructors use byte offsets. **Lessons**: these passes must be designed as a pipeline unit, not bolted on individually. Study GCC's SRA pass ordering and LLVM's MemorySSA for proper clobber tracking before reattempting. The alias analysis oracle (`alias.rs`) is correct and ready for use.
