...
fn collect_cli_warnings(opts: &mut Options, unknown_warning_flags: &[String]) {
    ...
    if opts.openmp {
        opts.cli_warnings.push(
            "-fopenmp enables preview OpenMP support; PARALLEL data environments, static DO worksharing, COLLAPSE(2), and scalar INTEGER/LOGICAL reductions are implemented and unsupported constructs or clauses are errors"
                .into(),
        );
