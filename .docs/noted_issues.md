...
- **(C)** Non-deterministic backend/codegen output across identical builds: repeated `-S` and repeated `-c` runs on the same source and opt level can emit different register choices and text bytes. `bencch` consistency suites also show `-S | as` vs `-c` object mismatches, but relocations/load commands/symbols stay stable while text changes. Blocks reproducible builds and “binary is exactly what we expect” confidence.
